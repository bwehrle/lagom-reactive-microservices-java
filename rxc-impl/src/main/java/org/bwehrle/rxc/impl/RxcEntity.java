...
/**
 * This is an event sourced entity. It has a state, {@link RxcState}, which
 ...
 */
public class RxcEntity extends PersistentEntity<RxcCommand, RxcEvent, RxcState> {
    /**
     * An entity can define different behaviours for different states, but it will
     * always start with an initial behaviour. This entity only has one behaviour.
     */
    @Override
    public Behavior initialBehavior(Optional<RxcState> snapshotState) {
        /*
        ...
        BehaviorBuilder b = newBehaviorBuilder(
                snapshotState.orElse(new RxcState("Hello", LocalDateTime.now().toString()))
        );
        ...
        b.setEventHandler(GreetingMessageChanged.class,
                ...
                // the event.
                evt -> new RxcState(evt.message, LocalDateTime.now().toString())
        );
